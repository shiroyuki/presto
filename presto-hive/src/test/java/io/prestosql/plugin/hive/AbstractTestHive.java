...
public abstract class AbstractTestHive
{
            ...
            throws Exception
    {
        ...
        try (Transaction transaction = newTransaction()) {
            ...
            Table oldTable = transaction.getMetastore().getTable(new HiveContext(session), schemaName, tableName).get();
            ...
            transaction.getMetastore().replaceTable(new HiveContext(session), schemaName, tableName, newTable.build(), principalPrivileges);
        ...
        try {
            ...
            try (Transaction transaction = newTransaction()) {
                ...
                Table table = transaction.getMetastore()
                        .getTable(new HiveContext(session), tableName.getSchemaName(), tableName.getTableName())
    ...
    public void testTableCreationIgnoreExisting()
    {
        ...
        try {
            try (Transaction transaction = newTransaction()) {
                ...
                Optional<Table> tableHandle = transaction.getMetastore().getTable(new HiveContext(session), schemaName, tableName);
    ...
    protected void testUpdateTableStatistics(SchemaTableName tableName, PartitionStatistics initialStatistics, PartitionStatistics... statistics)
    {
        ...
        assertThat(metastoreClient.getTableStatistics(context, tableName.getSchemaName(), tableName.getTableName()))
        ...
        for (PartitionStatistics partitionStatistics : statistics) {
            metastoreClient.updateTableStatistics(context, tableName.getSchemaName(), tableName.getTableName(), actualStatistics -> {
            ...
            throws Exception
    {
        ...
        HiveMetastore metastoreClient = getMetastoreClient();
        HiveContext context = new HiveContext(SESSION);
        Table table = metastoreClient.getTable(context, tableName.getSchemaName(), tableName.getTableName())
        ...
        List<PartitionWithStatistics> partitions = ImmutableList.of(firstPartitionName, secondPartitionName)
                ...
                .collect(toImmutableList());
        metastoreClient.addPartitions(context, tableName.getSchemaName(), tableName.getTableName(), partitions);
        metastoreClient.updatePartitionStatistics(context, tableName.getSchemaName(), tableName.getTableName(), firstPartitionName, currentStatistics -> EMPTY_TABLE_STATISTICS);
        metastoreClient.updatePartitionStatistics(context, tableName.getSchemaName(), tableName.getTableName(), secondPartitionName, currentStatistics -> EMPTY_TABLE_STATISTICS);
    }
            ...
            List<PartitionStatistics> secondPartitionStatistics)
    {
        ...
        HiveMetastore metastoreClient = getMetastoreClient();
        HiveContext context = new HiveContext(SESSION);
        assertThat(metastoreClient.getPartitionStatistics(context, tableName.getSchemaName(), tableName.getTableName(), ImmutableSet.of(firstPartitionName, secondPartitionName)))
        ...
        for (int i = 0; i < firstPartitionStatistics.size(); i++) {
            ...
            metastoreClient.updatePartitionStatistics(context, tableName.getSchemaName(), tableName.getTableName(), firstPartitionName, actualStatistics -> {
                assertThat(actualStatistics).isEqualTo(expectedStatisticsPartition1.get());
                return statisticsPartition1;
            });
            metastoreClient.updatePartitionStatistics(context, tableName.getSchemaName(), tableName.getTableName(), secondPartitionName, actualStatistics -> {
        ...
        metastoreClient.updatePartitionStatistics(context, tableName.getSchemaName(), tableName.getTableName(), firstPartitionName, currentStatistics -> {
            assertThat(currentStatistics).isEqualTo(expectedStatisticsPartition1.get());
            return initialStatistics;
        });
        metastoreClient.updatePartitionStatistics(context, tableName.getSchemaName(), tableName.getTableName(), secondPartitionName, currentStatistics -> {
            ...
            throws Exception
    {
        ...
        try {
            ...
            metastoreClient.addPartitions(context, tableName.getSchemaName(), tableName.getTableName(), ImmutableList.of(new PartitionWithStatistics(partition, partitionName, statsForAllColumns1)));
            assertEquals(
                    metastoreClient.getPartition(context, tableName.getSchemaName(), tableName.getTableName(), partitionValues).get().getStorage().getStorageFormat(),
                    fromHiveStorageFormat(ORC));
            assertThat(metastoreClient.getPartitionStatistics(context, tableName.getSchemaName(), tableName.getTableName(), ImmutableSet.of(partitionName)))
            ...
            Partition modifiedPartition = Partition.builder(partition)
                    ...
                    .build();
            metastoreClient.alterPartition(context, tableName.getSchemaName(), tableName.getTableName(), new PartitionWithStatistics(modifiedPartition, partitionName, statsForAllColumns2));
            assertEquals(
                    metastoreClient.getPartition(context, tableName.getSchemaName(), tableName.getTableName(), partitionValues).get().getStorage().getStorageFormat(),
            ...
            modifiedPartition = Partition.builder(partition)
                    .withStorage(storage -> storage
                            ...
                            .setLocation(partitionTargetPath(tableName, partitionName)))
                    .build();
            metastoreClient.alterPartition(context, tableName.getSchemaName(), tableName.getTableName(), new PartitionWithStatistics(modifiedPartition, partitionName, statsForSubsetOfColumns));
            assertThat(metastoreClient.getPartitionStatistics(context, tableName.getSchemaName(), tableName.getTableName(), ImmutableSet.of(partitionName)))
            ...
            modifiedPartition = Partition.builder(partition)
                    .withStorage(storage -> storage
                            ...
                            .setLocation(partitionTargetPath(tableName, partitionName)))
                    .build();
            metastoreClient.alterPartition(context, tableName.getSchemaName(), tableName.getTableName(), new PartitionWithStatistics(modifiedPartition, partitionName, emptyStatistics));
            assertThat(metastoreClient.getPartitionStatistics(context, tableName.getSchemaName(), tableName.getTableName(), ImmutableSet.of(partitionName)))
    ...
    protected String partitionTargetPath(SchemaTableName schemaTableName, String partitionName)
    {
        try (Transaction transaction = newTransaction()) {
            ...
            LocationService locationService = getLocationService();
            Table table = metastore.getTable(new HiveContext(session), schemaTableName.getSchemaName(), schemaTableName.getTableName()).get();
            ...
            throws Exception
    {
        ...
        try {
            ...
            HiveMetastore metastoreClient = getMetastoreClient();
            HiveContext context = new HiveContext(SESSION);
            metastoreClient.updatePartitionStatistics(context, tableName.getSchemaName(), tableName.getTableName(), "ds=2016-01-01", actualStatistics -> statistics);
            metastoreClient.updatePartitionStatistics(context, tableName.getSchemaName(), tableName.getTableName(), "ds=2016-01-02", actualStatistics -> statistics);
            ...
            throws Exception
    {
        ...
        try (Transaction transaction = newTransaction()) {
            ...
            HiveContext context = new HiveContext(session);
            Table table = getMetastoreClient().getTable(context, tableName.getSchemaName(), tableName.getTableName()).get();
            ...
            throws Exception
    {
        ...
        try (Transaction transaction = newTransaction()) {
            ...
            // verify table format
            Table table = transaction.getMetastore().getTable(new HiveContext(session), tableName.getSchemaName(), tableName.getTableName()).get();
            ...
            throws IOException
    {
        ...
        HiveContext hiveContext = new HiveContext(newSession());
        Set<String> existingFiles = new HashSet<>();
        for (String location : listAllDataPaths(hiveContext, transaction.getMetastore(), schemaName, tableName)) {
            existingFiles.addAll(listAllDataFiles(hdfsContext, new Path(location)));
        }
        return existingFiles;
    }

    public static List<String> listAllDataPaths(HiveContext context, SemiTransactionalHiveMetastore metastore, String schemaName, String tableName)
    {
        ImmutableList.Builder<String> locations = ImmutableList.builder();
        Table table = metastore.getTable(context, schemaName, tableName).get();
        ...
        Optional<List<String>> partitionNames = metastore.getPartitionNames(context, schemaName, tableName);
        if (partitionNames.isPresent()) {
            metastore.getPartitionsByNames(context, schemaName, tableName, partitionNames.get()).values().stream()
                    .map(Optional::get)
            ...
            throws Exception
    {
        ...
        try (Transaction transaction = newTransaction()) {
            ...
            HiveContext context = new HiveContext(newSession());
            List<String> partitionNames = transaction.getMetastore().getPartitionNames(context, tableName.getSchemaName(), tableName.getTableName())
            ...
            Map<String, Optional<Partition>> partitions = getMetastoreClient().getPartitionsByNames(context, tableName.getSchemaName(), tableName.getTableName(), partitionNames);
            ...
            throws Exception
    {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            try (Transaction transaction = newTransaction()) {
                ...
                // verify partitions were created
                List<String> partitionNames = transaction.getMetastore().getPartitionNames(new HiveContext(session), tableName.getSchemaName(), tableName.getTableName())
                        .orElseThrow(() -> new AssertionError("Table does not exist: " + tableName));
                assertEqualsIgnoreOrder(partitionNames, CREATE_TABLE_PARTITIONED_DATA.getMaterializedRows().stream()
                        .map(row -> "ds=" + row.getField(CREATE_TABLE_PARTITIONED_DATA.getTypes().size() - 1))
                        .collect(toList()));
        ...
        try (Transaction transaction = newTransaction()) {
            ...
            List<String> partitionNames = transaction.getMetastore().getPartitionNames(hiveContext, tableName.getSchemaName(), tableName.getTableName())
                    .orElseThrow(() -> new AssertionError("Table does not exist: " + tableName));
            for (String partitionName : partitionNames) {
                HiveBasicStatistics partitionStatistics = getBasicStatisticsForPartition(session, transaction, tableName, partitionName);
    ...
    private static HiveBasicStatistics getBasicStatisticsForTable(ConnectorSession session, Transaction transaction, SchemaTableName table)
    {
        return transaction
                .getMetastore()
                .getTableStatistics(new HiveContext(session), table.getSchemaName(), table.getTableName())
                .getBasicStatistics();
    }
    ...
    private static HiveBasicStatistics getBasicStatisticsForPartition(ConnectorSession session, Transaction transaction, SchemaTableName table, String partitionName)
    {
        return transaction
                .getMetastore()
                .getPartitionStatistics(new HiveContext(session), table.getSchemaName(), table.getTableName(), ImmutableSet.of(partitionName))
                .get(partitionName)
                .getBasicStatistics();
    }
    ...
    private void eraseStatistics(SchemaTableName schemaTableName)
    {
        HiveMetastore metastoreClient = getMetastoreClient();
        HiveContext context = new HiveContext(SESSION);
        metastoreClient.updateTableStatistics(context, schemaTableName.getSchemaName(), schemaTableName.getTableName(), statistics -> new PartitionStatistics(createEmptyStatistics(), ImmutableMap.of()));
        Table table = metastoreClient.getTable(context, schemaTableName.getSchemaName(), schemaTableName.getTableName())
                .orElseThrow(() -> new TableNotFoundException(schemaTableName));
        List<String> partitionColumns = table.getPartitionColumns().stream()
                .map(Column::getName)
                .collect(toImmutableList());
        if (!table.getPartitionColumns().isEmpty()) {
            List<String> partitionNames = metastoreClient.getPartitionNames(context, schemaTableName.getSchemaName(), schemaTableName.getTableName())
                    .orElse(ImmutableList.of());
            List<Partition> partitions = metastoreClient
                    .getPartitionsByNames(context, schemaTableName.getSchemaName(), schemaTableName.getTableName(), partitionNames)
                    .entrySet()
                    .stream()
    ...
    private void alterBucketProperty(SchemaTableName schemaTableName, Optional<HiveBucketProperty> bucketProperty)
    {
        try (Transaction transaction = newTransaction()) {
            ...
            Optional<Table> table = transaction.getMetastore().getTable(new HiveContext(session), schemaName, tableName);
            Table.Builder tableBuilder = Table.builder(table.get());
            tableBuilder.getStorageBuilder().setBucketProperty(bucketProperty);
            PrincipalPrivileges principalPrivileges = testingPrincipalPrivilege(tableOwner, session.getUser());
            transaction.getMetastore().replaceTable(new HiveContext(session), schemaName, tableName, tableBuilder.build(), principalPrivileges);
            ...
            throws Exception
    {
        ...
        try (Transaction transaction = newTransaction()) {
            ...
            List<String> partitionNames = transaction.getMetastore()
                    .getPartitionNames(new HiveContext(newSession()), tableName.getSchemaName(), tableName.getTableName())
                    .orElseThrow(() -> new AssertionError("Table does not exist: " + tableName));
            ...
            implements ConflictTrigger
    {
        ...
        public void triggerConflict(ConnectorSession session, SchemaTableName tableName, ConnectorInsertTableHandle insertTableHandle, List<PartitionUpdate> partitionUpdates)
        {
            ...
            HiveMetastore metastoreClient = getMetastoreClient();
            HiveContext context = new HiveContext(session);
            Optional<Partition> partition = metastoreClient.getPartition(context, tableName.getSchemaName(), tableName.getTableName(), copyPartitionFrom);
        ...
        public void verifyAndCleanup(ConnectorSession session, SchemaTableName tableName)
        {
            ...
            HiveMetastore metastoreClient = getMetastoreClient();
            HiveContext context = new HiveContext(session);
            Optional<Partition> actualPartition = metastoreClient.getPartition(context, tableName.getSchemaName(), tableName.getTableName(), toPartitionValues(partitionNameToConflict));
            ...
            metastoreClient.dropPartition(new HiveContext(session), tableName.getSchemaName(), tableName.getTableName(), conflictPartition.getValues(), false);
        }
    }
            ...
            implements ConflictTrigger
    {
        ...
        public void triggerConflict(ConnectorSession session, SchemaTableName tableName, ConnectorInsertTableHandle insertTableHandle, List<PartitionUpdate> partitionUpdates)
        {
            ...
            // Additionally, this method is not part of a test. Its purpose is to set up an environment for another test.
            HiveMetastore metastoreClient = getMetastoreClient();
            metastoreClient.dropPartition(new HiveContext(session), tableName.getSchemaName(), tableName.getTableName(), partitionValueToConflict, false);
        }
