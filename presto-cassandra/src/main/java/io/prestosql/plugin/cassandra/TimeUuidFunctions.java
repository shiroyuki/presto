...
public final class TimeUuidFunctions
{
    ...
    @Description("Cassandra minTimeuuid function")
    @ScalarFunction
    @SqlType("varchar(36)")
    public static Slice minTimeuuid(@SqlType("timestamp(3) with time zone") long timestamp)
    {
        long epochMillis = unpackMillisUtc(timestamp);
        return utf8Slice(UUIDs.startOf(epochMillis).toString());
    }

    @Description("Cassandra maxTimeuuid function")
    @ScalarFunction
    @SqlType("varchar(36)")
    public static Slice maxTimeuuid(@SqlType("timestamp(3) with time zone") long timestamp)
    {
        long epochMillis = unpackMillisUtc(timestamp);
